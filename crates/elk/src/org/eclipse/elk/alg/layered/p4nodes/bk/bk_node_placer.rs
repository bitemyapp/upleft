//! Port of `vendor/elk-swift/Sources/ElkSwift/ELK/org/eclipse/elk/alg/layered/p4nodes/bk/org_eclipse_elk_alg_layered_p4nodes_bk_BKNodePlacer.swift`.
//!
//! Not ported yet.

use crate::org::eclipse::elk::alg::layered::graph::l_graph::{LGraphArena, LGraphId};
use crate::org::eclipse::elk::core::alg::i_layout_phase::ILayoutPhase;
use crate::org::eclipse::elk::core::alg::i_layout_processor::ILayoutProcessor;
use crate::org::eclipse::elk::core::util::i_elk_progress_monitor::IElkProgressMonitor;

#[derive(Default)]
pub struct BKNodePlacer;

impl BKNodePlacer {
    pub fn new() -> BKNodePlacer {
        BKNodePlacer
    }
}

impl ILayoutProcessor for BKNodePlacer {
    fn process(&mut self, _lg: &mut LGraphArena, _graph: LGraphId, _monitor: &mut dyn IElkProgressMonitor) {
        unimplemented!("BKNodePlacer is not ported yet")
    }

    fn name(&self) -> &'static str {
        "BKNodePlacer"
    }
}

impl ILayoutPhase for BKNodePlacer {}
