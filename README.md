...
| DownrightApp and the command-line tools | `Sources/DownrightApp`, `drdownright`, `down`, `DownrightSpotlightMetadata`, `DownrightQL` | `upleft-app`, `upleft-cli`, `upleft-spotlight-metadata`, `upleft-quicklook`, `upleft-foundation` | app-layer suites identical; window captures identical | in progress. Done: the non-UI layer and the command-line tools. html-export 13706/13706 (6 themes × light and dark, plus print), spotlight 928/928, down-cli 244/244 (the real `down` against Upleft's, in sandboxed homes), workspace 6/6, find 4/4, palette 23/23, formats 30/30, updater 10/10, local-ai 4/4. All 15 `just app-bench` stages are as fast or faster (HTML export of the 5,000-line document: 0.58 ms vs 12.3 ms). The panels (all 37 `Panels/` files), built from scenarios off-screen: panel 228/228 (window-server pixels and view-tree dumps), panel-model 39/39 (589 states); every `just panel-bench` stage as fast or faster (task panel on the 5,000-line document 24.9 ms vs 32.9 ms, command palette ranking 21.9 ms vs 29.8 ms). Still to do: the windows and menus (UI port), and Sparkle linkage (packaging). See `crates/app/PORTING.md` and `crates/app/src/panels/PORTING.md`. |
