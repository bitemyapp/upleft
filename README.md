...
| MarkdownRender | `Sources/MarkdownRender` | `upleft-render` | `decorate` dump and `render` pixels identical | in progress. Done: the text view, container, content storage, fragment provider and motion, pixel- and layout-identical on the 558 prose-only documents in all 4 render variants and on every document in Source mode; the decoration engine and display map (decorate 5454/5454, incremental 2727/2727, displaymap 2727/2727, clipboard 909/909); the stylesheet, syntax highlighting and themes; the density gutter and its preview and outline windows (density-model 1818/1818; render-density 1126/1126 and density-hover 240/240 in view capture, screen capture unverified). Still to do: the object fragments (list ornaments, code blocks, callouts, tables, images, thematic breaks, front matter, math, Mermaid). |
...
| DownrightApp and the command-line tools | `Sources/DownrightApp`, `drdownright`, `down`, `DownrightSpotlightMetadata`, `DownrightQL`, `DownrightThumb` | `upleft-app`, `upleft-cli`, `upleft-spotlight-metadata`, `upleft-quicklook`, `upleft-thumb`, `upleft-foundation` | app-layer suites identical; window captures identical | in progress. Done: the non-UI layer and the command-line tools. html-export 13706/13706 (6 themes × light and dark, plus print), spotlight 928/928, down-cli 244/244 (the real `down` against Upleft's, in sandboxed homes), workspace 6/6, find 4/4, palette 23/23, formats 30/30, updater 10/10, local-ai 4/4. The Quick Look extensions (view controller, thumbnail provider, `.appex` bundles): quicklook-preview 16/16, quicklook-thumbnail 2745/2745, quicklook-thumbnail-sizes 154/154; whether the system loads them is unverified. All 15 `just app-bench` stages are as fast or faster (HTML export of the 5,000-line document: 0.58 ms vs 12.3 ms). Still to do: the windows, panels and menus (UI port), and Sparkle linkage (packaging). See `crates/app/PORTING.md`. |
